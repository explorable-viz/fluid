...
import Graph (GraphImpl, Vertex(..), emptyG, extend, fwdSlice, inE, starInOut)
...
graphTestScript = do
   let
      ...
      graph = foldl (\g α -> extend (Vertex (show α)) (fromFoldable $ map (Vertex <<< show) [ α + 2, α + 3 ]) g) emptyG ids :: GraphImpl
   let
