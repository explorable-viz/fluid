...
import Graph (GraphImpl, Vertex(..), emptyG, fwdSlice, inE, starInOut, union)
...
graphTestScript = do
   let
      ...
      graph = foldl (\g α -> union (Vertex (show α)) (fromFoldable $ map (Vertex <<< show) [ α + 2, α + 3 ]) g) emptyG ids :: GraphImpl
   let
