...
import { DataType, PrimType, ctrFor, ctrToDataType, exprSuffix, initDataType, types } from "./DataType"
...
export function eval_ (ρ: Env, e: Expr): ExplValue {
   ...
   if (e instanceof Expr.DataExpr) {
      ...
      return explValue(Expl.dataExpl(c, tv̅.map(({t}) => t))(kₜ), at(ctrFor(c).C, ...tv̅.map(({v}) => v))(kᵥ))
   } else
