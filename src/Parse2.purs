...
import Expr2 (Expr(..), thunkSugar) as E
import Lattice (Raw)
...
expr_ = fix $ appChain >>> buildExprParser ([ backtickOp ] `cons` operators binaryOp)
   ...
   backtickOp = flip Infix AssocLeft $ do
      ...
      pure (\e e' -> E.Sugar $ E.thunkSugar $ BinaryApp e x e')
   ...
   binaryOp op = do
      ...
      onlyIf (op == op') $
         ...
         else \e e' -> E.Sugar $ E.thunkSugar (BinaryApp e op e')
   ...
   appChain expr' = simpleExpr >>= rest
      ...
      simpleExpr =
         ...
         record = sepBy (field expr') token.comma <#> E.Record unit # token.braces
         ...
         lambda = E.Lambda <$> (keyword str.fun *> branches expr' branch_curried)
