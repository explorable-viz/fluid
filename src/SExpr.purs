...
desugarModuleFwd :: forall a m. MonadError Error m => JoinSemilattice a => Module a -> m (E.Module a)
...
moduleFwd :: forall a m. MonadError Error m => JoinSemilattice a => Module a -> m (E.Module a)
...
varDefFwd :: forall a m. MonadError Error m => JoinSemilattice a => VarDef a -> m (E.VarDef a)
varDefFwd (VarDef π s) = E.VarDef <$> pattContFwd π (ContNone :: Cont a) <*> desug s

-- VarDefs
varDefsFwd :: forall a m. MonadError Error m => JoinSemilattice a => VarDefs a × Expr a -> m (E.Expr a)
...
recDefsFwd :: forall a m. MonadError Error m => JoinSemilattice a => RecDefs a -> m (E.RecDefs a)
...
-- RecDef
recDefFwd :: forall a m. MonadError Error m => JoinSemilattice a => RecDef a -> m (Bind (Elim a))
...
-- Expr
exprFwd :: forall a m. MonadError Error m => JoinSemilattice a => Expr a -> m (E.Expr a)
...
-- ListRest
listRestFwd :: forall a m. MonadError Error m => JoinSemilattice a => ListRest a -> m (E.Expr a)
...
-- List Qualifier × Expr
listCompFwd :: forall a m. MonadError Error m => JoinSemilattice a => a × List (Qualifier a) × Expr a -> m (E.Expr a)
listCompFwd (α × Nil × s) =
...
-- Clauses
clausesFwd :: forall a m. MonadError Error m => JoinSemilattice a => Clauses a -> m (Elim a)
