...
dims =
   ...
   op (Val α _ (Matrix (MatrixRep (_ × MatrixDim (i × β1) × MatrixDim (j × β2)))) : Nil) = do
      v1 <- new (val' None) (singleton β1) $ Int i
      v2 <- new (val' None) (singleton β2) $ Int j
...
matrixUpdate =
   ...
   op (Val α _ (Matrix r) : Val _ _ (Constr c (Val _ _ (Int i) : Val _ _ (Int j) : Nil)) : v : Nil)
      | c == cPair = new (val' None) (singleton α) (Matrix (matrixPut i j (const v) r))
...
dict_difference =
   ...
   op (Val α _ (Dictionary (DictRep d)) : Val β _ (Dictionary (DictRep d')) : Nil) =
      new (val' None) (singleton α # Set.insert β) (Dictionary (DictRep (d \\ d')))
...
dict_disjointUnion =
   ...
   op (Val α _ (Dictionary (DictRep d)) : Val β _ (Dictionary (DictRep d')) : Nil) = do
      new (val' None) (singleton α # Set.insert β) (Dictionary (DictRep (disjointUnion d d')))
   op _ = throw "Dictionaries expected"
...
dict_intersectionWith =
   ...
   op (v : Val α _ (Dictionary (DictRep d1)) : Val α' _ (Dictionary (DictRep d2)) : Nil) = do
      ...
      new (val' None) (singleton α # Set.insert α') v'
      where
      apply' (β × u) (β' × u') = do
         v''@(Val _ _ key) <- G.apply v u >>= flip G.apply u'
         Val β'' _ _ <- new (val' None) (singleton β # Set.insert β') key
         pure (β'' × v'')
...
dict_map =
   ...
   op (v : Val α _ (Dictionary (DictRep d)) : Nil) = do
      ...
      new (val' None) (singleton α) (Dictionary (DictRep d'))
