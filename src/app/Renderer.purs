...
foreign import drawFigure :: MatrixRep' -> MatrixRep' -> MatrixRep' -> Effect Unit
...
renderFigure :: Val 𝔹 × Val 𝔹 -> Val 𝔹 × Val 𝔹 -> Val 𝔹 × Val 𝔹 -> Effect Unit
renderFigure (input × input') (filter × filter') (output × output') =
   ...
   in drawFigure (bits (input'' × fst (match input')))
