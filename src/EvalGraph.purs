...
match :: forall s m. Monad m => Val Vertex -> Elim Vertex -> MayFailT m (Env Vertex × Cont Vertex × s Vertex)
match v (ElimVar x κ)
...
matchMany :: forall s m. Monad m => List (Val Vertex) -> Cont Vertex -> MayFailT m (Env Vertex × Cont Vertex × s Vertex)
...
closeDefs :: forall s m. Monad m => Env Vertex -> RecDefs Vertex -> s Vertex -> WithGraphAllocT s m (Env Vertex)
closeDefs γ ρ αs =
   flip traverse ρ \σ ->
      let
         ρ' = ρ `for` σ
...
{-# Evaluation #-}
apply :: forall s m. Monad m => Val Vertex -> Val Vertex -> WithGraphAllocT s m (Val Vertex)
...
eval :: forall s m. Monad m => Env Vertex -> Expr Vertex -> s Vertex -> WithGraphAllocT s m (Val Vertex)
...
eval_module :: forall m s. Monad m => Env Vertex -> Module Vertex -> s Vertex -> WithGraphAllocT s m (Env Vertex)
