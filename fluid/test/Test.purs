...
tests :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => TestSuite m
tests = allTests

scratchpad :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => TestSuite m
scratchpad = second void <$> suite paragraph_cases (1 × false)

filterSuite :: forall m r. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => Array String -> Array { file :: String | r } -> SuiteFactory r m -> TestSuite m
filterSuite files cases makeSuite =
   second void <$> makeSuite (filter (\c -> c.file `elem` files) cases) (1 × false)

allTests :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => TestSuite m
allTests = concat (benchmarks <#> asTestSuite) <> linkingTests <> illFormedTests

linkingTests :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => TestSuite m
linkingTests = linkedOutputsSuite linkedOutputs_cases <> linkedInputsSuite linkedInputs_cases

illFormedTests :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => TestSuite m
...
benchmarks :: forall m. MonadAff m => MonadError Error m => HasCxt m => HasModuleStore m => MonadReader FileCxt m => LoadFile m => Array (BenchSuite m)
benchmarks =
   [ suite desugar_cases
   , suite misc_cases
