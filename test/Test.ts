...
describe("example", () => {
	...
	describe("lookup", () => {
		...
		it("parses ok", () => {
			runTest(__nonNull(file.text), Profile.Parse, τ.some(τ.arg(τ.str(τ.endArgs(null)))))
		})
	})
	...
	describe("zipW", () => {
		...
		it("ok", () => {
			runTest(__nonNull(file.text), Profile.Parse, τ.cons(τ.arg(τ.pair(τ.arg(τ.int(τ.arg(τ.int(τ.endArgs(τ.var_(τ.endArgs(null)))))))))))
		})
	})
})
